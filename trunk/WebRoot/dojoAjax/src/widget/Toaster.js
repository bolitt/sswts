...
showDelay:2000, postCreate:function () {
	...
	if (this.positionDirection.match(/^[tb]r-/)) {
		...
		try
		{		
			outer = dojo.byId(this.outerNodeName);
		}catch(e){}
		
		var outerwidth = view.width;
		if(outer)
		{
			outerwidth=outer.clientWidth;
		}
		if(outerwidth > view.width)
		{
			outerwidth = view.width;
		}
		this.clipNode.style.left = ((view.width + outerwidth) / 2 - nodeSize.width - 1 - scroll.left) + "px";
		//this.clipNode.style.left = (view.width - nodeSize.width - 1 - scroll.left + parseInt(this.leftbias)) + "px";
	} else {
