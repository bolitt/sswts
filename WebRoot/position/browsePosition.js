var BrowsePosition ={
	getPosURL : "position.do?method=getAllPosition",
	showPosDetailURL : "position.do?method=showPositionDetail"
};

BrowsePosition.init = function() {
	dojo.html.setOpacity("posDetail",0);
	loading(true);
	dojo.io.bind({url:BrowsePosition.getPosURL,preventCache : true, handler:function(type, data, evt) {
		loading(false);
		if(basicHandler(type, data, evt)) {
			...
			BrowsePosition.generateTree(node, schooldata);
			var deptdata = bigdata[1];
			node = dojo.byId("DeptPos");
			BrowsePosition.generateTree(node, deptdata);
			var orgsdata = bigdata[2];
			node = dojo.byId("OrgsPos");
			BrowsePosition.generateTree(node, orgsdata);
		}
	}, content:{}});
};


BrowsePosition.generateTree = function(node, data) {
	...
	var reporter = function(reporter) 
	{
		...
		this.go = function(message) {
			if(message.node.postNo) {
				BrowsePosition.showPosDetail(message.node.postNo);
			}
		};
	};		
...
BrowsePosition.showPosDetail = function(posNo) {
	document.body.style.cursor="wait";
	dojo.io.bind({
		url:BrowsePosition.showPosDetailURL, 
		preventCache : true,
		...
		content:{posnum:posNo}
	});
}

content = dojo.widget.byId("main");
content.onLoad = BrowsePosition.init;
